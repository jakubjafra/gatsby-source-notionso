...
function lengthAttributeSequence(items, index, att) {
    let count = 0;
...
function removeAttributeFromItem(item, attName) {
    const ix = item.atts.findIndex(a => a.att === attName);
    if (ix < 0) {
        throw new Error(`could not find attrubute ${attName} in ${JSON.stringify(item)}`);
    }
    item.atts.splice(ix, 1);
}
// may be a method exists in the std lib for Array
function extractSubSlices(items, from, to) {
    const result = [];
...
function renderSlices(items, renderFuncs) {
    ...
    items.forEach((item, index) => {
        ...
        item.atts.forEach(a => (counter[a.att] = lengthAttributeSequence(items, index, a.att)));
    });
    ...
    if (max[0] === 1) {
        ...
        return items.map(item => {
            return item.atts.reduce((child, a) => {
    ...
    result.push(renderFuncs.renderTextAtt(renderSlices(subslices, renderFuncs), attName));
...
function renderTextItems(items, renderFuncs) {
    return renderSlices(items, renderFuncs);
}
function renderNotionText(input, renderFuncs, renderUtils, debug = false) {
    const result = [];
    ...
    splits.forEach(({ ref, items }) => {
        const children = renderTextItems(items, renderFuncs);
        if (ref === '') {
...
exports.default = renderNotionText;
//# sourceMappingURL=renderNotionText.js.map